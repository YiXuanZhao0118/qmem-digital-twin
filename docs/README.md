...
### 7.6 Tapered Amplifier（設計鎖定 2026-05-31）
半導體增益晶片，種子放大。鎖定：forward A→B only；只放大 gain-axis 偏振分量（輸出沿 `gainAxisDegBodyLocal` 線偏振 + 有限消光）；ASE 僅在無種子時發射（option 6b）。飽和增益律：`P_out = P_sat·ln(1 + (P_coupled/P_sat)·(G0−1))`，`G0=10^(gainDb/10)`，clamp 到 outputPowerMaxMw。Op 在 `anchor_ops/misc_ops.py`，代表資產 `toptica_boosta_pro`（face A 種子 −z / B 輸出 +z）。已知 bug：op 讀 `smallSignalGainDb` 但舊資產存 `gainLinear`，待統一。
