...
- **per-port 編輯**：DDS `rf_out` 行可改 Freq/Vpp；AOM `rf_in` 顯示 incoming + **η%**（見下）；amp 顯示 gain 與 Vpp_in→Vpp_out。**channels 的讀寫都走所有權鏈**（2026-08-14 修）：顯示用 `channelsByObjectId`（`RfLinkPanel.tsx:918`）解 `dynamicSources` > Asset `defaultParams` > `kindParams`，寫入用 `updateSceneObject` 打進 **`dynamicSources.channels`**（與 `Ad9959ObjectControls.updateChannel` 同一條路）。**不可寫 `kindParams`** —— ad9959 asset 自己作者了 `channels`（且標 tunable），asset 層永遠贏過 kindParams，於是「面板數字變了、傳播沒變」，下游 AOM 死守 asset 的 80 MHz / 1.0 Vpp。
- **AOM `rf_in` 的 η 讀數與 solver 同源（2026-08-14 修）**：`aomDriveReadout`（`RfLinkPanel.tsx:344`）直接呼叫 `optical/kinds/aom/physics.ts` 的 `diffractionEfficiency` / `rfPowerForPeakEfficiencyW` —— 就是 tracer 跑的 datasheet 模型（`η = baseEfficiency·sin²((π/2)√(P/P_peak(λ)))·G(f)`，`P_peak(λ)=rfPowerForPeakW·(λ/peakRefWavelengthNm)²`）。三個對齊點缺一不可：① **參數層**用 asset `defaultParams` + `dynamicSources`（tracer 看得到的那份），不是 PhysicsElement `kindParams`；② **波長**取場景 emitter 的 `centerWavelengthNm`（`sceneWavelengthNm`，多種波長才退回常數）—— `P_peak ∝ λ²`，780 vs 852 差 19%；③ **G(f) 的設計中心**維持 AOM 自己的 `centerFreqMhz`（傳 `designCenterFreqMhz`），否則把載波當成中心 → G 恆為 1，調頻不會扣效率。badge 改用 `η / baseEfficiency` 分級（<0.5 under、<0.9 off-peak、否則 OK），因為過驅動會越過 sin² 反曲點、功率越大效率越低，用 Vpp 門檻表達不出來。
...
✅ **AOM 那列的 `⚠ under` 是假警報（2026-08-14）**：RF Link 面板對 AOM0 標「need ≥ 92.68 V ⚠ under」，但 solver 當下跑的是 η ≈ 0.82（接近 0.85 峰值）。根因＝面板自己用一套**舊的閉式公式**（`aomRequiredPowerW`：`P = W·λ²/(2·M²·L²)`，取 `figureOfMeritM2` / `acousticBeamWidthMm`）算門檻，而且**波長寫死 780 nm**；那些參數只存在 PhysicsElement `kindParams`，**tracer 根本不讀**（anchor op 只看 asset `default_params` + `dynamic_sources`），公式本身也不是 tracer 用的 datasheet 模型。於是「面板說驅動不足、實際上已接近滿效率」。修法＝整段換成呼叫 `optical/kinds/aom/physics.ts` 的共用函式（見 §3），面板不再自己算物理。現場數字：`P_peak(852.347 nm) = 2.2·(852.347/1100)² = 1.3209 W → 22.99 Vpp`（舊值 92.68 Vpp），`η = 0.8151 → 顯示 82%`，與後端 `first_order_efficiency` 逐位相同。**通則**：面板要顯示物理量就呼叫 kind 的 physics 模組，不要在 UI 檔裡另寫一份公式 —— 它一定會跟 solver 漂移，而且漂移時使用者只看得到 UI 那份。
