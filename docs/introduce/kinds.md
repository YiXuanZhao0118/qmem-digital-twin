...
**9 個接頭 Asset3D（alembic `0115`）**：`fiber_connector` 下 5 列（`fiber_connector_{apc,pc}_{pm,sm}` + `fiber_connector_pc_mm`，共用 FC STL、依 0061「always clone, never share」每列獨立 row，`default_params` 帶 polish/polishAngleDeg/fiberType/mfd/na/core/cladding/slowAxisKeyed/returnLossDb，`wavelengthRangeNm` 走 column）；`rf_cable_connector` 下 4 列（`rf_connector_{sma,bnc}_{male,female}`，`file_path = primitive://{family}_{gender}_connector`，`default_params` 帶 family/gender/tipMm/impedanceOhm/maxFreqGhz/couplingType）。各列 `anchors` 為完整 tri-axis frame：`connect_out`(原點,axisX −X)、`connect_in`(在 tipMm, axisX +X；fiber 帶 apertureMm 0.125)。`tunable_params=[]`、`locked=false`。**程序渲染已 gender-aware**（`connectorModels.ts` `RfConnectorKind = {sma,bnc}_{male,female}`、`proceduralRfConnector()` 派發四個 builder；`normalizeRfConnectorKind` 把舊 family-only `"sma"/"bnc"` 正規化成 male＝沿用舊渲染；`DigitalTwinViewer` 的 resolver 改指向真實 `rf_connector_{kind}` 資產，primitive:// 則回 null→程序模型，**移除了把接頭畫成 pbs252 placeholder 的舊 hack**）。**仍待 0116**：per-end gender 進資料模型（現 `endAConnector` 只存 family）、binding-tree 渲染 forwarding、connect_in/out anchor 驅動擺放取代 `bakeConnectorFrame` bbox 啟發式。遷移是唯一 seed 路徑（`seed_v3_assets.py` 是手動且已過時的 script，非 fresh-install 路徑）。
