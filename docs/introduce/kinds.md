...
每 kind 契約：前端註冊渲染器（`kinds/<kind>/`）+ 後端物理（**live：`optical/anchor_ops/<kind>.py` 的 anchor op**；face-based `optical/kinds/<kind>/physics.py` 已退役）+ `kinds.json` 參數。代表性 defaultParams：laser 852.347nm/50mW（像散,2026-06-11 起 X↔Y 對調為 X 0.52µm/Y 2.2µm）；AOM v=4200 m/s、n=2.26、baseEfficiency 0.85；glan_polarizer wedge 38.5°、ER 55dB；TA gain 30dB、sat 500mW。lens 預設薄透鏡（`focalLengthMm`）；asset 另帶 `radiusFrontMm`+`refractiveIndex`+`centerThicknessMm` 時自動切**厚透鏡 ABCD**（短焦/非球面，如 A230TM-B，見 [optics.md](optics.md)）。
