...
Fibres and pigtails linked to a deleted object keep their dangling `fiberEndpoints` / `pigtailEndpoints` link; rigid groups move together but do not delete together. A requested id with no row counts as success — it is reported in `deletedObjectIds` and nothing cascades from it.

**Nothing is gated by kind inside the cascade** — an rf_cable or PPG is deleted when the cascade reaches it, or when it is asked for by name (RF Link's Disconnect does exactly that). The gate is at the ENTRY POINTS: the Outliner has no row for them and the Object panel no Remove button (`capabilityProfile.outlinerVisible` / `showRemoveObjectButton`, both false for `rf_cable` and `programmable_pulse_generator`), and **since wave 3b the Delete key checks the same two flags** (`App.tsx:146`). It used to delete any unlocked selection, so clicking a cable in the viewport and pressing Delete removed it — and, by the orphan rule, the PPG on it — which is the out-of-band deletion those profiles exist to prevent, and which [timing.md](timing.md) already said was impossible.
