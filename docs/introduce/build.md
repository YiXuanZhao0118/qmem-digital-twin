...
BUILD 上選的 **kind 決定資產的 domain**（`kind.domains` 為權威；不選 kind ＝ mechanical／無物理）。domain rail 分桶現已 kind-authoritative，BUILD 早期殘留的 `properties.domains=["mechanical"]` 不再把已 kinded 的資產困在機械軌。
