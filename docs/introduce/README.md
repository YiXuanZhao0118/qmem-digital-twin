...
- [Surface optics (plan) · ../surface-optics.md](../surface-optics.md) — tracing free-space parts through their real faces (Snell, Fresnel, curved surfaces, birefringent media) instead of one anchor plane; Phases 0–2 (the `assets_3d.surface_model` column, the engine in `backend/app/optical/surfaces/`, the tracer wiring) landed 2026-09-23; Phase 3 done for 23 parts (every free-space part but the AOM, which keeps its op; the A230TM collimator from Thorlabs' Zemax file, moved to its real spacing, DBR mode re-fitted); Phase 4 begun (PHY Editor shows surface models)
