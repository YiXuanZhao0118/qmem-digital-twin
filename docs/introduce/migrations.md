...
- **Head：** `0113_param_ownership_tunable`；**Root：** `0001_initial_schema`。
- 單一線性鏈，無 gap、無分支、無重複 revision id。`env.py` 以 `from app.models import Base` 取 metadata 當 autogenerate target。
- revision id 刻意縮寫以守 **VARCHAR(32)** 限制（曾有 0091 超限把 DB 卡在 0090 的事故，現已健康；最新 `0113_param_ownership_tunable` = 28 字元）。
