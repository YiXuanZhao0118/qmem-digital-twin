...
- **L1** the gizmo (Global / Local / Beam orientation, TransformControls) — see the rotation magnet and the translate-axis auto-hide below.
...
- **L4** optical tools (Place / Insert along beam; the last-clicked beam point `scopeProbe`). Also **Mirror coupling** — select two steering mirrors and solve both poses so the beam hits each at exactly 45°, centred, and leaves on a destination port's axis. Unlike everything else on this page it sets ROTATION as well as position, and it is the only tool that writes two mirrors plus the pass-through optics in one batch. The closed form, the both-beams-touch-both-mirrors precondition and the free DOF a U-turn leaves behind are in [mirror-coupling.md](mirror-coupling.md).
...
### Translate-axis auto-hide + handle spread (translate mode only)

An arrow seen down its own axis is useless but still **pickable**: TransformControls' axis picker is a cone whose wide end sits at the gizmo origin, so end-on it projects onto the gizmo centre *in front of* the XY/YZ/XZ quads and steals their clicks. Dragging it there is worse than useless — the drag plane is built from `eye × axis`, which degenerates as the eye approaches the axis, so the plane turns edge-on and one pixel of pointer travel becomes an enormous displacement. Stock `AXIS_HIDE_THRESHOLD` is 0.99 (~8° off-axis), too tight to cover "roughly a front or top view". Two patches in `frontend/src/three/placement/gizmo.ts`, both pure + unit-tested (`three/placement/__tests__/axisVisibility.test.ts`, `handleSpacing.test.ts`):

- **`axisVisibilityForView(eye, gizmoQuat)`** (`gizmo.ts:95`) drops an axis at `|axis · eye| > AXIS_HIDE_DOT = 0.97` (~14° off-axis, `gizmo.ts:89`) — wide enough for a front/top view, narrow enough that a mildly tilted view still offers all three. Hiding an axis hides its pickers, so it also stops stealing clicks. `eye` is the gizmo→camera direction (for an `OrthographicCamera`, the negated view direction instead — a pivot-relative eye is meaningless there); the frame is identity in world space and the proxy's world quaternion in local space.
- **`PlacementGizmo.updateAxisVisibility()`** (`gizmo.ts:461`) is the per-frame driver. ⚠️ **Invariant: it must run BEFORE `renderer.render`** — it is called from `animate()` (`DigitalTwinViewer.tsx:3478`) because the gizmo re-reads `showX/Y/Z` in its own `updateMatrixWorld` *during that same draw*, so deciding here makes an axis appear on the frame the camera crosses the threshold rather than one frame late. It restores all three axes in rotate/scale mode (the rings stay usable end-on) and **while a drag is live**, so a drag never loses the axis it is holding.
- **`spreadTranslateHandles(controls)`** (`gizmo.ts:123`) fixes the same overlap from the other side, once at construction: the plane quads move outward by `PLANE_HANDLE_OFFSET = 0.15` (0.15 → 0.30, i.e. spanning 0.20–0.40, still inside the 0.5-long axis line) and the axis *pickers* are scaled by `AXIS_PICKER_THINNING = 0.5` perpendicular to their own axis, so a quad's inner corner is no longer buried inside the cone. ⚠️ `setupGizmo()` bakes each handle's transform **into its geometry** and resets the node transform every frame, so geometry is the only durable place to move a handle — a `handle.position.set(…)` would be wiped on the next frame. It reaches into the three-internal `_gizmo` and **no-ops (leaving stock behaviour) if a three upgrade renames it**, rather than throwing.
...
3. **Batched cable write-back** (`resnapRfCablesLinkedTo`): this used to be one PATCH + commit per cable per endpoint (3 cables = 6). It became one `updateSceneObjects(..., { recordHistory: false })` folding End A and End B of each cable into a single properties payload, and since wave 3b it is a single `POST /api/v3/rf-cables/resnap` — the backend sweeps every linked end in one transaction and answers with the rows it changed, which the store upserts in one commit. It stays a **derived** write, recording no undo entry: the move that triggered it already recorded one, and undoing that move re-runs this pass.
