...
**The same align on the backend (2026-09-22)**: `POST /api/v3/pigtails/{id}/candidates | apply | disconnect` and `/api/v3/pigtails/resnap` ([api.md](api.md)) are ports of the four store actions — `pigtail_candidates` / `pigtail_apply_candidate` / `pigtail_clear_link` / `pigtail_resnap` in `backend/app/optical/fibers/service.py` (:303 / :337 / :415 / :434), `pigtailPortBindings` as `optical/fibers/scene.py:344`, and `pigtailAlignment.ts` transcribed from three.js in `optical/fibers/pigtail.py` (the `Matrix4` / `Euler` decompositions branch, so they are copied, not re-derived). `apply` writes the same three things the web does — the connector binding's `ObjectBinding` delta (all six axes, angles wrapped to (−180, 180], override asset and row properties carried over), the jacket's last node in `properties.bindingFiberNodes[bindingId]`, and `properties.pigtailEndpoints[portAnchor]` — in one transaction, and never moves the instrument. Pinned to the TypeScript by `frontend/src/utils/__tests__/fiberParity.test.ts` (the helpers at arbitrary poses, the store flows step by step), and against the tracer's own `_port_connector_anchors` by `backend/tests/optical/test_fiber_endpoints.py`: after an apply, the re-seated `intercept_out` sits one mating gap upstream of the receptacle. The receptacles a pigtail can plug into are placed through the tracer's transform chain on both sides (binding tree + SceneObject pose, since 2026-09-22) — see [fiber.md](fiber.md#the-backend-port-2026-09-22). Two data corners follow the web rather than the loader and are harmless today: with two bindings claiming one `portAnchor` the first wins (the loader's re-seat takes the last), and an `ObjectBinding.asset_3d_id_override` on a connector binding is not honoured.
