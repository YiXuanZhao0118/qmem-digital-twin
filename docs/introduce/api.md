...
**The coax-cable and Programmable Pulse Generator flows, for every client.** They began (2026-09-22) as backend ports of the web app's `store/sceneStore.ts`, so the qmem-blender add-on would not grow a third copy; in wave 3b the web app was pointed at them and its TypeScript deleted, so this is now the only implementation. Routers `backend/app/routers/v3_rf_cables.py` / `v3_ppg.py`; pure plans `backend/app/optical/rf_cables/flows.py`; DB side `rf_cables/service.py`. The web calls them from `frontend/src/api/client.ts` (`rfCableConnectApi`, `rfCableDisconnectApi`, `rfCableResnapApi`, `rfCableAlignCandidatesApi`, `rfCableAlignApi`, `ppgAttachApi`) — one call per store action, plus an optimistic local update. `backend/tests/fixtures/rf_cables/{pure,flows}.json` are kept as the **frozen goldens the real TypeScript wrote** before it was deleted, asserted by `backend/tests/optical/test_rf_cables_parity.py` (1e-9 on poses / nodes, exact on the cable variant, the PPG component, rule rejections and delete sets), so a Python change still has to be a deliberate fixture update. Behaviour and the quirks carried over are in [rf.md](rf.md) §7 and [cable.md](cable.md). **The endpoint tests write to the real database**: `backend/tests/optical/test_rf_cables_endpoints.py` and `test_rf_ports_binding_chain.py` seed a bench, call the endpoints, and tear down through `tests/optical/rf_bench_cleanup.py` (`purge_bench`), which sweeps by LINK — every object whose `rfCableEndpoints` / `ppgAttachment` names a bench object, to a fixpoint, plus those PPGs' TimingPrograms — not by Component. On the dev database (`tests/conftest.py` defaults `DATABASE_URL` to :55432) `plan_connect` / `plan_ppg_attach` take the catalog's own `RF cable SMA` / `RF cable BNC SMA` / `PPG BNC MALE` (first match in scene order) over the bench's, so a component-keyed sweep left 9 cables + 5 PPGs behind per run (69 + 38 dangling by 2026-09-24, see [rf.md](rf.md) §7), and for the same reason the tests pin the picked cable's end families and the PPG's `connectorType` + usable asset, not the bench's row. `backend/scripts/purge_rf_orphans.py` audits the live scene for a cable with an end, or a PPG with an attachment, naming an object that is gone and, with `--apply`, unlocks and deletes them through `POST /api/v3/objects/delete` (dry-run first; aborts if the cascade would reach anything else).
