...
3. **The LOD1/LOD2 pipeline** — **P0 and P1 are done (2026-08-17)**; P2 remains:
   ...
   - **P2 (backfill)** — generate tiers for the existing catalog. Writes only the new table, so locked rows stay untouched. **Until an asset has tiers it renders LOD0 at every distance**, so the feature is currently inert on the shipped catalog — this backfill is what actually turns R-5 on.
