...
**B.7 also done same day** — Touchstone upload + Smith chart + S-param dB plot
...
| B.7 | scikit-rf Touchstone parser + Smith chart + S-param plot | ✅ | (this commit) | `pip install scikit-rf>=1.1`。新 backend `services/touchstone.py` (parse_touchstone wraps `skrf.Network`,輸出 freq + nPorts + z0 + sParams dict {sNM: [[re,im], ...]});新 router `POST /api/touchstone/parse` (multipart upload, 4 MB cap, 422 on bad file)。3 個 unit tests pass。新 frontend `modules/electronics/SmithChart.tsx`(pure SVG, constant-resistance circles + reactance arcs + reflection trace + start/end markers);`NetworkAnalysisPanel.tsx`(file upload → POST → SmithChart S11/S22 + uPlot magnitude (dB) plot for all Sij with log-frequency X axis);掛在 ElectronicsWorkspace results pane 底部。新 1 個 e2e (synthetic .s2p upload → assert SmithChart + magnitude plot 出現)。**Live verify**:browser 上傳 5-point 1–5 GHz .s2p → networkMeta 「2-port, Z₀=50 Ω, 5 pts, 1.00 GHz → 5.00 GHz」、Smith chart 含 S11+S22 traces、magnitude plot canvas 渲染、no console errors。258 backend pytest + 7 playwright e2e 全 pass。 |
...
**Estimate:** 6–10 週(全職)
**Started:** 2026-05-12
**Mock-mode done:** 2026-05-12(同日完成)
**Backend real palace dispatch wired:** 2026-05-12(asyncssh + palace_io + em_fem auto-switch)
**C.8 vtk.js field viewer:** 2026-05-12(mock 16³ Gaussian volume rendering)
**Workstation install + .pvtu pull-down:** ⏭ User-side install per `docs/PHASE_C_WORKSTATION_SETUP.md`,實際 .pvtu 串流 endpoint 留 Phase C.8+ follow-up
...
| C.3 | Backend: `SshWorkstationRunner` (asyncssh) | ✅ | 2773441 (stub) + (this commit) (real) | Phase C.4 swap:stub → real asyncssh-backed runner。`submit` opens session via `_run_in_background` (same pattern as InProcessRunner) — actual SSH protocol invoked from inside `solvers/em_fem.py` 's `_run_real_palace_via_ssh`,not the runner itself,so future modules can reuse runner with their own SSH workflow。 |
| C.4 | Workstation setup:palace + Gmsh + runner agent | ✅(backend wiring) / 🔒(install) | (this commit) for backend wiring;workstation install user-side | Backend C.4 done:`pip install asyncssh`;新 `solvers/palace_io.py` (build_palace_config 從 EmProblem 產 palace JSON config,parse_palace_sparams 解 palace `port-S.csv` mag/phase format → 標準 `[re, im]` dict);`em_fem.py` auto-switch (sim_run.runner_kind == 'ssh_workstation' AND settings.workstation_host set → SCP mesh + config / `docker run --rm awslabs/palace:latest /work/config.json` 60min timeout / SCP `port-S.csv` 回來 / parse → result_summary;否則 fall back mock)。新 5 個 palace_io 單元測試 pass (config + parser)。**Workstation install 仍由 user 手動完成** — 詳細步驟見 `docs/PHASE_C_WORKSTATION_SETUP.md` (sshd setup / docker pull palace / SSH key 設置 / .env config / smoke test)。 |
| C.5 | Backend: `solvers/em_fem.py` adapter | ✅ | 2773441 (mock) + (this commit) (real palace branch) | Mock branch (Phase C.5,跑在 inproc) 不變。新 real branch `_run_real_palace_via_ssh` 走 SSH:asyncssh.connect → mkdir remote temp → SCP mesh+config → docker run palace → SCP port-S.csv 回 → parse → result_summary 同 Phase B.7 shape。Auto-switch 邏輯在 `run()` 內(check `sim_run.runner_kind` + `settings.workstation_host`)。 |
...
| C.8 | Frontend field viewer:vtk.js volume rendering | ✅ | (this commit) | `npm install @kitware/vtk.js`;新 `modules/em/FieldViewer.tsx` 用 vtkFullScreenRenderWindow + vtkVolumeMapper + vtkColorTransferFunction (dark blue→magenta→yellow) + vtkPiecewiseFunction (low alpha→0.4 max);auto-range 從 data min/max。Mock palace 產 16³ Gaussian blob (`_mock_field_payload` in em_fem.py) 餵進 viewer。Real palace 暫時 emit `field.available=false` placeholder;Phase C.8+ 補 .pvtu 串流 endpoint。`EmFieldPayload` discriminated union type;EmRunResultPreview 條件 mount FieldViewer。**Browser verify**:click EM tab → New → Run → 4-6 秒 status='completed' + Smith chart + magnitude plot + volume rendering canvas (title "\|E\|² (mock, peak ~5.5 GHz)") 同時渲染。E2e 加 field viewer 斷言。 |
