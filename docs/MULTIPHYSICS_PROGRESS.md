...
| B.3 | ngspice binary install + path config in `settings` | 🚧 | — | User 跑 `choco install ngspice -y`(elevated PS)。安裝後 backend 重啟讓 PATH 生效,e2e POST {module:'spice'} 應該 status='completed'。`settings.ngspice_path` config 已加。 |
