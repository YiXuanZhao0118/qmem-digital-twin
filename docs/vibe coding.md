...
| `collections`、`collection_members` | outliner 用的 collection 樹狀結構 | 一個 object 只屬於一個 collection (alembic 0016)；flag 只剩 `visible`（沿 hierarchy 級聯）與 `rigid_transform`（true 時整個 sub-tree 的 objects 視為一個 rigid group，移/旋轉一個就同步全部 — 0035）。**Lock 是純 per-SceneObject** (`objects.locked`)；outliner 上的 collection lock icon 是 bulk action（toggle 所有後代 objects.locked），不存任何 collection-level 狀態 |
...
| **0035** | collection_rigid | 2026-05-11：清掉 `collections` 上四個沒人用的 boolean (`locked` / `exclude` / `holdout` / `indirect_only`) — `locked` 從未被 enforce、後三者是 Blender outliner 直接照搬但對 optical scene 沒語意。同步 drop `scene_view_collection_overrides` 的 `exclude` / `holdout` / `indirect_only`（保留 `visible`）。新增 `collections.rigid_transform bool default false`：true 代表整個 sub-tree 的 objects 是 rigid group，移動/旋轉任一成員 → 套同樣 rigid-body transform 給其他成員。Effective state 在 read time 沿 ancestor 算 cascade（child 繼承 parent 的 true），不存。Lock 改成純 per-SceneObject — outliner 上的 collection lock icon 是 bulk action over descendants，不存任何 collection-level state |
...
| `fiber` | 整條光纖 patch cable（雙向）：`fiberType`、`endA/endB: FiberEndSpec`（aperture / NA / MFD / connector / polish / Fresnel residual / glass index / **PM slow axis**）、`cutoffWavelengthNm`、`operatingWavelengthRangeNm`、`designWavelengthNm`、`maxInputPowerMw`、`attenuationCurve[]`、`bendLoss: BendLossConstants`、`minBendRadiusMm`、`birefringenceDeltaN` (PM)、`pmdCoefficientPsPerSqrtKm`、`polarizationExtinctionRatioDb` (PM)、`bandwidthMhzKm` (MM)。**KIND_REGISTRY.fiber 合約 (2026-05-12)**：`requiredAnchors = ["intercept_in", "intercept_out"]`、`anchorsNeedingDirection = ["intercept_in", "intercept_out"]`、`anchorsNeedingAperture = ["intercept_in", "intercept_out"]`、`alignVariant = "none"`（走自訂 align，不走通用 dispatcher）。direction 由 `Anchor.derivedFromFiberEndpoint` flag 從 spline tangent 自動算（不是讀 `directionBodyLocal`），contract 仍標記 direction-bearing 是因為 align 語意要求。 | 自定 align：`alignFiberEndToBeam(componentId, "A"\|"B", toleranceMm=25)` **投影光學 PORT**（intercept_in / intercept_out，= ferrule tip 在 spline node 外 36.28 mm 處）到最近 beam segment，然後反推新的 spline node 位置 = projected_port − new_outward · 36.28 mm（End A entry: outward = −beam_tangent；End B exit: outward = +beam_tangent）。只動該端，內部節點不動。**2026-05-12 修 bug**：之前投影 spline node 而非 port，造成 port 落在 beam 線之外 36.28 mm（方向對，位置差一個 ferrule 長度）。詳見 §16 「Fiber 系統（Phase A–J）」 |
| `isolator` | `forwardLossDb`、`isolationDb`、`transmissionAxisDegBeamLocal`。**Per-template override (2026-05-12)**：catalog Components 可帶 `properties.isolatorKindParamsOverride: { forwardLossDb, isolationDb }`，由 `default_kind_params_for_component` deep-merge 到 DEFAULT_KIND_PARAMS["isolator"]（同 fiber 套路）。Catalog 端還會在 `properties` 一併存 human-readable 規格 `centerWavelengthNm`、`tuningRangeNm`、`transmissionPercent`、`isolationDb`、`isolationDbRange`、`maxBeamDiameterMm`、`maxPowerW`、`housing`、`isTandem`。所有 29 條 Thorlabs NIR 690–1080 nm isolators（IO-/IOT-/I*P3D 系列）由 `seed.py` 的 `_ISOLATOR_SPECS` 表 + `_build_isolator_meta()` helper 注入（forwardLossDb = -10·log10(T) 從 Thorlabs 公布 transmission % 推得） | translate intercept_in → beam |
...
| `three/rayTrace.ts` | 前端 ray tracer：emission、reflection、refraction、Jones polarization、AOM diffraction（Phase 7.4 改用 `diffractedDirection` + `expectedInputDotD2` from physics.ts、sign 跟 alignToLaser 結構性一致）、TA gain、~1700 行。**PBS coating-normal V2 read path fix（2026-05-11）**:V2 Phase 6（alembic 0032）把 PBS 內部對角鍍膜的法線從 `OpticalElement.kindParams.coatingNormalBodyLocal` 搬到 `SceneObject.properties.anchorBindings[opticalSurface].payload.normalBodyLocal`，但 rayTrace.ts 的 `SPLITTING_KINDS` 分支沒同步 update reader —— 還在 `kp?.coatingNormalBodyLocal ?? kp?.coatingNormalLocal` 找，找不到就 fallback 到 `worldNormal`（cube 外殼面的法線，跟入射光方向幾乎平行），導致反射 = `reflectDirection(dir, worldNormal)` 變成 180° back-reflection，反射光退回 laser 軸上跟入射光重疊，visually 看起來「沒有反射光」。**Fix**:先從 `hitObj.properties.anchorBindings[].find(b => b.kind === 'opticalSurface')?.payload?.normalBodyLocal` 讀 binding（V2 source of truth），讀不到才 fallback `kindParams.coatingNormalBodyLocal` → `coatingNormalLocal` 兩個 legacy 欄位。對 canonical PBS（normalBodyLocal = (0.707, 0.707, 0)）+ beam 沿 +X 入射，現在反射方向 = `(1,0,0) − 2 × 0.707 × (0.707, 0.707, 0) = (0, −1, 0)` → 反射光沿 lab −Y 垂直射出，物理正確。Verified: PBS 在 (250, 0, 500)，from-PBS 兩條 seg 為 transmitted(250,0,500)→(517,0,500) + reflected (250,0,500)→(250,−1000,500)，畫面上可以同時看到水平的穿透光 + 垂直的反射光 |
...
| `utils/fiberAnchorResolver.ts` | Fiber port anchor 解析：當 `Anchor.derivedFromFiberEndpoint` 設置時，從 spline endpoint + outward·`FIBER_FERRULE_TIP_MM` 即時算 anchor 的有效 body-local pos/dir，render / ray-trace / align 全部走同一個 resolver 保證一致。**export `FIBER_FERRULE_TIP_MM = 36.28`**（30126A9 STL housing length 的 canonical 值，被 `rayTrace.ts` 跟 `fiberAlignment.ts` import）|
| **`utils/fiberAlignment.ts` (2026-05-12)** | `computeFiberEndAlignment({ end, nodes, pose, beamSegmentsLab, toleranceMm })` — pure 幾何 helper，背後支撐 `sceneStore.alignFiberEndToBeam`。投影 PORT（不是 spline node）到最近 beam segment、反推新 node 位置 = projected_port − new_outward·`FIBER_FERRULE_TIP_MM`、保留 handle magnitude 只改方向。Export `endpointOutwardBody(nodes, end)` 給 caller 同樣 −handle/|.| fallback to neighbour 的規則。Self-contained rotation math (`R_z·R_x·R_y`) 不依賴 store/THREE，方便 vitest 測試（`utils/__tests__/fiberAlignment.test.ts` 13 tests covering A entry / B exit / handle fallback / out-of-tolerance / pose translation / 90° Z rotation）|
...
| **`utils/rigidGroup.ts`** | Collection.rigidTransform cascade resolver。`resolveRigidGroup(scene, objectId)` 走 collection ancestors 找最高一層 `rigidTransform=true` ancestor，把它 sub-tree 下的所有 SceneObject id 收進 set。`expandPoseToRigidGroup(scene, leading, patch)` 把 leading object 的 pose patch 展開成整個 rigid group 的 patch list — `ΔR = q_new ⊗ q_old⁻¹` (world-space rotation delta) + `ΔT = pos_new − pos_old`，每個 member 的新 pose = `pos_old_leading + ΔR.apply(pos_member − pos_old_leading) + ΔT`、新 quaternion = `ΔR ⊗ q_member`、然後反解 YXZ Euler 回 (rxDeg, ryDeg, rzDeg)。**Lock policy**：group 內任一 non-leading member 是 `locked` → `kind: "rejectedLockedMember"`，整個 transform reject（不部分套用，不破 rigid invariant）。`computeRigidCollectionIds(collections)` 給 outliner 算「effectively rigid」(self OR any ancestor rigidTransform)，render inherited indicator 用。`store/sceneStore.ts.updateSceneObject` 在 `safePatch` 還有 pose 改動時呼叫 `expandPoseToRigidGroup`，group 走 `Promise.all(updateObjectApi)`、single 走原本 single path |
| `utils/...` | 其他 helper |
...
| `components/DigitalTwinViewer.tsx` | ★ 主 3D viewport（含 marquee 選取、placement gizmo、wireframe outline、port labels、AOM tilt-axis 箭頭（Phase 7.1：取代舊的 ABC sphere markers，渲染 b̂×â 自動推導的 Bragg 旋轉軸 = 跟 alignToLaser 用的同一條）、waveplate fast-axis 指示線）。**Click handler — beams unpickable on single-click**：單擊只走 `pickObject`；object miss → `selectObject(null)` deselect，**不再 fallback 到 `pickBeam`**。Beam 點擊是 OpticalLinkViewerPanel 的 dedicated UX，避免在 main scene 點目標元件時被前面的 beam tube 攔走。雙擊保留 `pickBeam` 設精準 scope probe（power-user 手勢，故意一點才會觸發）。**On-demand render（2026-05-09）**：`animate` loop 用 `cameraMoved \|\| pendingRender` gate `renderer.render(...)` + `orientationRenderer.render(...)`，`cameraMoved = controls.update()`（OrbitControls damping 還沒停就持續 true）。`requestRenderRef`（指向 `() => { pendingRender = true }` closure）由 `controls` 'change' 事件、gizmo `onDraggingChange` / `onDragUpdate`、`handlePointerMove`（hover highlight 之後）、`handlePointerLeave`、`resize` 直接呼叫；元件尾端 no-deps `useEffect(() => requestRenderRef.current?.())` 是 safety net，每次 React commit fire 一次，捕捉兄弟 useEffect 對 scene 的副作用 mutation。Wall-fade per-frame loop + orientation-gizmo quaternion update 搬進 gated 分支。Idle = 0 fps，camera move / drag / hover / state change 自動補 render。**Incremental rebuild（2026-05-09）**：rebuild useEffect 不再 `clearGroup(componentGroup) + 全 reload`，改用 `objectWrappersRef = useRef<Map<objectId, {wrapper, componentRef, assetRef, stateRef}>>` 做 reference-equality cache：`(component, asset, deviceState)` 三件 ref 都跟上次一樣 → cache hit、只 `stripDynamicDecorations(wrapper)` + 重貼 transform/displayMode/decoration；任何一件 ref 變了 → cache miss、dispose 舊 wrapper + 重新 `await loadAssetObject(...)`。Beam + relation group 還是 full rebuild。Asset object 加 `userData.isLoadedAsset = true` 給 cache-hit path 找回原物件呼叫 `applyObjectGeometryOffset`。`stripDynamicDecorations` 透過 `userData.isOutline / isPortLabel / isAomTiltAxisMarker` + 四個 `relation-*` named child 找可拆 decoration，asset 主體保留。Effect cleanup 只 set `cancelled = true`，不再 clearGroup（會弄壞 cache）；init unmount 才整個 dispose + `objectWrappersRef.current.clear()`。Cleanup 移除 'change' listener + reset ref to noop。**Move-to-Beam paint fix（2026-05-11，雙層 requestRender）**:cache-hit path 用 `applyObjectTransform(wrapper, effectivePlacement)` 改 three.js scene 但 *不* 自動設 `pendingRender = true`,以前要靠 no-deps safety-net useEffect 補一筆 render — 但 `renderComponents()` 是 async IIFE,React commit 後 await 還沒 resolve,safety-net 已經跑過了(`pendingRender` 也已被前一個 animate frame 清掉),於是新 transform 卡在 scene 直到下一次 mouse hover / F5。**Fix 兩層**:(1) 每一次 `applyObjectTransform / applyViewerDisplayMode / applyVisibilityFlags / decorate` 之後,在 `for (const placement of sceneData.objects)` 的迴圈裡直接 call `requestRenderRef.current?.()` — 確保每個物件的 transform mutation 都立刻留下 pending render,即使後續 `if (cancelled) return` bail 也沒關係;(2) async IIFE 結尾(`renderRayTraces()` + `setComponentsBuildVersion` 之後,未 cancelled 時)再 call 一次 — cover ray-trace + beam group rebuild。**為什麼要兩層**:drag-from-library 會觸發 backend `auto_create_optical_element_for_object` + anchor binding bootstrap,WS broadcast 連續多筆(Object + OpticalElement + properties patch)很常見,中間幾次 useEffect 的 async IIFE 會被連續 cancelled,只有最後一次 complete;沒有第 (1) 層的話,中間被 cancel 的 useEffect 雖然已經把 wrapper transform 設好,但 IIFE 因 cancelled flag bail 在 `setComponentsBuildVersion` 之前 → 沒人 fire requestRender → wrapper 動了但 canvas 永遠不重畫,直到 F5 才看到正確 solid scene。第 (1) 層讓「transform 已經 mutate 完成」這件事跟「IIFE 是否完整跑完」解耦。Affects "Align to Beam" 跟所有透過 `updateSceneObject(...)` 只動 pose 的操作(原本對 PBS / mirror / waveplate / EOM / TA / laser_source 全部一視同仁卡同樣 bug,因為 cache-key 是 component+asset+state ref equality,跟 element kind 無關;2026-05-11 用程式化測試在 beam_splitter / eom / tapered_amplifier / laser_source 四個 kind 上驗證 wrapper move + render 同步成功)。**Beam clearGroup race fix(2026-05-11)**:同一波修正裡另一個獨立 bug — `clearGroup(beamGroup) + clearGroup(relationGroup)` 原本在 useEffect 一開始就 sync 跑,但 `renderRayTraces() + renderRelations()` 卻在 async IIFE 裡被 `if (cancelled) return;` gate;`updateSceneObject` 會連續產生兩波 store update(manual `set()` from API response + WS `applyEvent` broadcast),導致 run 1 在 await 中被 run 2 cancel → run 1 bail before renderRayTraces,beamGroup 已被 clearGroup 清空。Run 2 雖然會重跑,但如果再被 run 3 cancel,beams 就永久消失到 F5。**Fix**:把 `clearGroup(beamGroup) + clearGroup(relationGroup) + renderRayTraces() + renderRelations()` 一整塊移到 async IIFE 內,放在 cancelled check 之後 — 被 cancel 的 run 完全不動 beam/relation group,只有完整跑完的 run 會「原子地」clear+repopulate,失敗就保留前一張 frame 的 beams。Symptom 是 user 看到「新增 PBS 移動到 beam 後反射光不見了,只有 F5 後才回來」 |
...
| `components/optical/OpticalLinkViewerPanel.tsx` | 光路圖 panel:選一個 emitter(laser_source / standalone tapered_amplifier),在 FloatingPanel 內畫獨立 mini 3D viewport 顯示該 emitter 的整條 beam chain(Gaussian taper cylinder + anchor spheres + aperture rings),點 beam tube 設 `scopeProbe` + 在下方 inline 顯示 BeamScopeContents 圖。資料源是 `window.__rayTraceDebug`(由 DigitalTwinViewer 發佈),用 `setInterval(tick, 16)` poll,內部 cache `prevContentKey` 避免重貼一樣的場景。**Lazy-mount fix(2026-05-11)**:[FloatingPanel.tsx#L139](qmem-digital-twin/frontend/src/components/workspace/FloatingPanel.tsx#L139) 在 `!layout.visible` 時 `return null`,所以 panel 預設(`defaultLayout.visible: false`)時整段 inner DOM(含 mount div)根本不存在。OpticalLinkViewerPanel 的 Three.js 初始化 useEffect 原本是 `deps: []`,App mount 時跑一次、看到 `mountRef.current === null` → `if (!mount) return;` bail,之後 user 第一次開 panel 雖然 mount div 出現了,但 useEffect deps 沒變所以不會 re-fire — 結果 user 看到空白,F5 後因為 panel.visible 已經是 true、mount div 在初始 render 就存在所以 useEffect 順利跑完。**Fix**:用 `const panelVisible = usePanelLayout("optical-link-viewer").visible;` 訂閱可見性,把 `panelVisible` 加進 useEffect deps;visibility false→true 觸發 useEffect re-run、mount div 此時已 mounted、renderer 建得起來。Close panel → visibility true→false → useEffect cleanup 拆掉 renderer + dispose Three.js resources;Reopen 再次 re-run、建新 renderer。Verified close/open cycle:canvas count 3→2→3。順帶保留 `firstRealResize` flag(invalidates `prevCameraFitKey` on first non-zero `mount.clientWidth`)當第二道防線,避免 layout reflow 跟 useEffect 早一拍 fire 的競爭 |
| `components/optical/...` | 其他 optical UI |
| `components/OutlinerPanel.tsx` | 場景 outliner（collection 樹狀）。**Per-row icons**：`Eye` (visibility cascade，session-only) / `Lock` (per-object 鎖；object row 上的 lock toggle 直接讀寫 `SceneObject.locked`，鎖了 trash button 自動 disabled、`stripLockedTransformPatch` 也會剝掉 pose patch) / `Trash`。**Per-collection icons**：`Eye` / **`Lock` (bulk三態)** / **`Link2` (rigid_transform toggle)** / `FolderPlus` / `Pencil` / `Trash`。Bulk lock 三態用 `collectionLockState(id)` 算 `all` / `none` / `mixed` / `empty`：`all` 顯示 active locked icon（click → unlock all 後代）；`none` 顯示 unlock icon（click → lock all 後代）；`mixed` 顯示 muted lock icon（click → lock all）；`empty` disabled。**沒有 collection-level lock state** — 純 bulk action over `collectAllObjectIdsUnder()` 拿到的後代 SceneObject ids，每個都呼叫 `updateSceneObject(id, { locked: next })`。`Link2` 顯示 own / inherited / off 三狀態，own 透過 `collection.rigidTransform`、inherited 透過 `computeRigidCollectionIds` 算 effective set；click → `updateCollection(id, { rigidTransform: !current })`。Master collection 的 rigid toggle disabled（root 不需要 rigid 整顆 scene）。**Frozen-group warning**：當 effective rigid 的 collection 後代含任何 locked object，Link2 加 `.warning` class（amber 色）+ tooltip「Rigid group is frozen — N member(s) locked. Unlock to move.」— 用戶看 outliner 一眼就知道為什麼 gizmo 拖不動 group。**Drag**：`DragPayload.kind="object"` 是 `objectIds: string[]`，single-row drag 包單顆 id，row 在 `selectedObjectIds` 裡時 drag 整個 selection（multi-select group move）。Locked ids 在 `dragableIds = sourceIds.filter(!locked)` pre-filter 掉；剩 0 → `draggable={false}` 整個 row 不能 drag。Drop handler `Promise.all(moveObjectToCollection)` over 整個 list（store 端再 silent-skip 一次 locked、後端 `move_object_to_collection` 也回 409，三層 defense） |
| `components/ComponentPanel.tsx` | 左側 component library + 右側 Object FloatingPanel。內部用 `isObjectSelection = Boolean(selectedObject)` 切兩種模式：object-selected 時顯示 transform / OpticalElementPanel / `TimingEditorButton`（laser_source / TA / AOM / EOM 才 enabled）；component-only 時只顯示 component name、CapabilityPills、3D model iframe。`TimingEditorButton` 被 `isObjectSelection &&` gate，component-only 模式不顯示；component spec JSON dump section 已移除。當所選物件 `componentType === "text_annotation"` 時，Object panel 多渲一段 `<TextAnnotationEditor>`（textarea 內容 / Width mm / Font px / 三顆 color picker），所有欄位寫到 component properties，sprite 透過 `loadAsset.createTextAnnotation` rebuild |
...
| `routers/objects.py` | SceneObject CRUD + scene-level move/rotate/scale endpoints。**Lock enforcement**：`strip_locked_transform_updates` 在 PUT 時剝掉 pose 欄位 (`x_mm` / `y_mm` / `z_mm` / `rx_deg` / `ry_deg` / `rz_deg`)、DELETE 上 `if scene_object.locked: 409 Conflict`（前端 `store.deleteObject` 跟 ctx menu 都已先 filter 掉 locked，這個 409 是 defense-in-depth 給直接打 API 的 path） |
...
| `routers/collections.py` | outliner collections (move-not-link 模型)。**Lock enforcement**：`move_object_to_collection` 在 reparent 前檢 `scene_object.locked` → 409 Conflict。Locked = 全面凍結（pose mutation / delete / reparent 三條 path 全擋），不會分裂成「凍結 pose 但可以換 home」的混合語意 |
...
- **Port disk (2026-05-12 fix)**：在 connector group local `(0, FIBER_FERRULE_TIP_MM, 0)` 加一個 Ø2.5 mm `CircleGeometry` disk,normal = +Y(=outward),APC 額外繞 local X 軸 tilt 8° 對齊 STL housing 的 8° 斜面。**Invisible render**(`material.colorWrite=false, depthWrite=false`)但仍可被 raycast hit — fix 一個 critical bug:Thorlabs 30126A9 FC housing STL 是個沒有端面 cap 的中空殼,完美對軸的光線(lab y=0, z=500 走 +x)直接從中央軸穿過去 0 個三角形,raycaster 找不到 hit,fiber dispatch 不會 fire。Disk 補上這個 cap。`userData.fiberRole = "portDisk"`,`fiberPolish = PC/APC`
...
### Align action（Phase F，2026-05-12 fix + extract）

`sceneStore.alignFiberEndToBeam(componentId, "A"|"B", toleranceMm=25)` 現在是 thin wrapper，純幾何邏輯在 `utils/fiberAlignment.ts` 的 `computeFiberEndAlignment(...)`。Store 端負責收集 nodes / pose / beam segments（legacy `beamPaths[]` + live `__rayTraceDebug`，三 → lab mm 由 inverse swap `lab_x = three.x·100, lab_y = -three.z·100, lab_z = three.y·100` 完成）、餵給 helper、把回傳結果寫進 `nodes[idx]` 再走 `updateFiberNodes`。

Helper 演算法：
1. **算當前 port lab 位置** = node + outward · `FIBER_FERRULE_TIP_MM`（outward 跟 `fiberAnchorResolver.endpointOutwardBodyLocal` 同規則：−handleOut 在 End A、−handleIn 在 End B，handle 缺位則 fallback 到 neighbour direction）
2. 對每條 beam segment 投影 **port** 到線段，clamp 在端點之間，取距離最小的；若所有 segment 都 > toleranceMm 就 return null
3. **反推新 spline node** = projected_port − new_outward · `FIBER_FERRULE_TIP_MM`，其中 new_outward 在 End A entry 為 −beam_tangent（face 對撞 beam），End B exit 為 +beam_tangent（face 順著 beam）
4. 改該端 handle 的 direction（保留 magnitude）使 outward = new_outward（End A: handleOut = +beam_tangent；End B: handleIn = −beam_tangent）
- 中間節點完全不動
- panel 兩個按鈕「對齊 A 端到 beam」「對齊 B 端到 beam」綁這個 action
- **2026-05-12 修 bug**：pre-fix 是投影 spline node（步驟 1 用 nodeLab 而非 portLab），造成 port 還是落在 beam 線外 36.28 mm（方向對，位置差一整個 ferrule 長度）。user 的明確需求是 port 跟 beam 重合，所以改成投影 port 並反推 node。
- **2026-05-12 extract**：純數學從 `sceneStore` 拆到 `utils/fiberAlignment.ts`，store 端只剩 thin wrapper（收集 input + commit output）。同時把 `36.28` 集中到 `utils/fiberAnchorResolver.FIBER_FERRULE_TIP_MM` 一個 export，`rayTrace.ts` 跟 `fiberAlignment.ts` 都 import 它（之前三處重複 hardcode）。改 FC 連接頭幾何只要改 `FIBER_FERRULE_TIP_MM` 一個地方。
- **Vitest 守護**：`utils/__tests__/fiberAlignment.test.ts` 13 tests 鎖死兩個 invariant — (I1) 對齊後 port 落在 projected beam point 上、(I2) 對齊後 |node − port| = `FIBER_FERRULE_TIP_MM` 沿 new outward 方向。涵蓋 End A entry / End B exit / handle 缺位 / 超 tolerance return null / 沒 beam return null / pose translation / 90° Z rotation。
...
### Frontend ray-trace fiber dispatch（`three/rayTrace.ts`, 2026-05-12 fix）

當 raycaster 命中一條 fiber 的 tube/housing/portDisk:
1. **不**透過 `asset.anchors.find(...)` 取 intercept_in/out — fiber 共用 `primitive_box` Asset3D,該 asset 的 anchors 是別的 component 帶過來的(常見:isolator-style `(0, ±55, 0)`、aperture 17.5 mm),用了會把 fiber 的光學 port 認錯位置。
2. 直接從 spline 算 port body-local 位置:`portA = node[0] + endpointOutwardBody("A") · FIBER_FERRULE_TIP_MM`,`portB = node[last] + endpointOutwardBody("B") · FIBER_FERRULE_TIP_MM`。`endpointOutwardBody` 從 `utils/fiberAlignment.ts` import,規則跟 `fiberAnchorResolver.endpointOutwardBodyLocal`、align 行為、PHY Editor 預設值全部對齊。
3. body → lab 用 SceneObject pose,再投到 three world frame 比 hitPoint 距離決定 entry/exit。
4. Coupling η_mode (Marcuse Gaussian overlap) × η_fresnel × η_attenuation 計算後,從 exit port 沿 exit outward 方向發新 beam,光路繼續。

組合 fix:port disk(§ render)補 raycast 命中,resolver bypass 用 spline 算正確 port pose,兩件事缺一不可才能讓「laser 進 fiber 出 fiber」的 round-trip 走通。
