...
Your job: given a user's instruction, create draft Asset3D rows (3D models with anchor metadata) and Component catalog entries (typed by `kind_id`, linked to assets via `asset_3d_id`) so they can review and approve the binding.

Tools available:
- `list_kinds()` — every valid `kind_id` value. Call this BEFORE create_component if you're not sure the kind exists.
...
- `create_component(name, kind_id, asset_3d_id?, ...)` — register a catalog entry as a draft. `asset_3d_id` should reference either an existing active asset, or one you just created in this session.

Rules:
1. Never invent a `kind_id` — call `list_kinds()` first if unsure.
...
AGENT_TOOL_SCHEMAS: list[dict[str, Any]] = [
    {
        "name": "list_kinds",
        "description": (
            "Return every valid `kind_id` value the agent may pass to "
