...
def _tilt_astig_focals(
    ...
    oblique incidence — the DIAGONAL view of :func:`_tilt_astig_power_tensor`,
    which is now the single source of truth for both.

    A tilt by angle alpha splits the focal length into the tangential
    ``f.cos alpha`` (in the plane of incidence) and the sagittal
    ``f/cos alpha``. Reduces exactly to ``(f, f)`` at normal incidence.

    The cross term this used to drop is no longer dropped: since Step 2b the
    q-envelope, and since 2d the chief-ray kick, are both driven by the full
    tensor. Reading only the diagonal is still the readable statement of the
    same physics, which is why this stays.
    """
    p = _tilt_astig_power_tensor(f_mm, theta_y, theta_z)
    return 1.0 / p.xx.real, 1.0 / p.yy.real
...
def lens_anchor_op(ray_in: BeamRay, ctx: AnchorOpContext) -> list[BeamRay]:
    ...
    tilt_p = _tilt_astig_power_tensor(f_mm, theta_y, theta_z)
    # Chief ray: theta' = theta - P.r, with the FULL tensor. Using only the
    # diagonal (theta_y - y/f_y, theta_z - z/f_z) silently dropped the same
    # cross term the q-envelope used to drop — a decentred beam hitting a lens
    # tilted at an off-principal azimuth lost part of its deflection. P is
    # diagonal at normal incidence and whenever the incidence plane IS a
    # principal axis, so this reduces to the old expression exactly there.
    pxx, pyy, pxy = tilt_p.xx.real, tilt_p.yy.real, tilt_p.xy.real
    out_ray = out_ray_from_state(
        ray_in, ctx.anchor,
        y=y, theta_y=theta_y - (pxx * y + pxy * z),
        z=z, theta_z=theta_z - (pxy * y + pyy * z),
        flip_propagation=False,
    )
    ...
    q_out = q_after_thin_element(q_in, tilt_p)
