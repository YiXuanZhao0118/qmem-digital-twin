...
| Per-kind physics | `physics/PhysicsElementPanel.tsx`, `AomAdjustControls.tsx`, `TaperedAmplifierAdjustControls.tsx`, `SimpleAdjustControls.tsx`, `_shared.tsx` | Kind-specific inspectors, extracted from a previously 4000-line monolith |
...
| Per-instance state | `SceneObject.properties.kindParams` + `objectBindings` | `objects.dynamic_sources` only (alembic 0082; `param_overrides` dropped in 0113 — per-instance editing is now gated by `assets_3d.tunable_params`) |
