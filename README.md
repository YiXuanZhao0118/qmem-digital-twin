...
| `rfCableAnchorResolver.ts` | Same for RF cables (spline + connector-tip math + the render-time re-derive of a linked end). `rfCableAlignment.ts` used to sit beside it; the align / connect / resnap maths is `POST /api/v3/rf-cables/*` now |
