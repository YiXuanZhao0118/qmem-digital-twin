...
describe("emissionFromObject", () => {
  it("uses ObjectPanel Lab Sense pose plus Asset/CAD-local anchor frame", () => {
    const placement = {
      xMm: 100,
      yMm: 200,
      zMm: 300,
    ...
    expect(result.origin.x).toBeCloseTo(101, 6);
    expect(result.origin.y).toBeCloseTo(202, 6);
    expect(result.origin.z).toBeCloseTo(303, 6);
