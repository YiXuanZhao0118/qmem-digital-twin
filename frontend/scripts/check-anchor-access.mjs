#!/usr/bin/env node
// Guard: forbid raw access to body-frame anchor fields outside the
// canonical helpers. The recurring bug fixed on 2026-05-27 (anchor
// fields read raw and composed with SceneObject pose without applying
// body frame) is hard to spot in code review, so we enforce by grep.
//
// Allowed files (raw access OK):
//   - utils/anchorAccess.ts            (the canonical helper)
//   - utils/assetFrame.ts              (low-level body-frame math)
...
//   - optical/ray-tracer-v3.ts         (V3 in-browser tracer, body-frame work)
//   - optical/kinds/**/physics.ts      (per-kind physics — body-frame by design)
//   - optical/registry.ts              (kinds metadata)
//   - kinds/isolator/pbsOverlay.ts     (PBS overlay reads anchor raw — pure body-frame placement)
...
// keeps body-frame for physics-internal D1/D2/D3 derivation).
